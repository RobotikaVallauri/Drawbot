...
void keyPressed()
{
  ...
  if (!streaming) {
    if (keyCode == LEFT) Muovi(0,-speed,0);
    if (keyCode == RIGHT) Muovi(0,speed,0);
    if (keyCode == UP) Muovi(speed,0,0);
    if (keyCode == DOWN) Muovi(-speed,0,0);
...
void Muovi(float x,float y,float z)
{
px+=x;
py+=y;
pz+=z;
